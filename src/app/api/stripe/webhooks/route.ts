...
export async function POST(req: Request) {
  ...
  try {
    event = stripe.webhooks.constructEvent(
      await (await req.blob()).text(), // Get raw body text from request
      req.headers.get("stripe-signature") as string, // Stripe signature header for verification
      process.env.STRIPE_WEBHOOK_SECRET as string // Secret key used to verify the event
    );
  } catch (error) {
    ...
    console.log(`❌ Error message: ${errorMessage}`); // Log the error message for debugging Stripe webhook verification issues
